...
# --- 2. KHỞI TẠO SESSION STATE (PHẢI ĐẶT TRÊN CÙNG ĐỂ TRÁNH ATTRIBUTE ERROR) ---
if 'dau' not in st.session_state:
    ...
    st.session_state.dang5 = [0]*5
    st.session_state.cl4 = [0]*4
    st.session_state.bt4 = [0]*4
    # 8 Biến phụ 50/50
    ...
    st.session_state.n1 = 10
    st.session_state.n2 = 36
    st.session_state.rd = 0
    st.session_state.rk = 0
    st.session_state.rg = 0
    st.session_state.ls = []
...
def cap_nhat_logic():
    ...
    # Tính hạng B trước khi cập nhật khan
    m_scores = get_matrix_scores()
    df_rank = pd.DataFrame(m_scores).sort_values(by=["d", "s"]).reset_index(drop=True)
    hạng = df_rank[df_rank['s'] == f"{n:02d}"].index[0] + 1
    st.session_state.ls.insert(0, {"Số về": f"{n:02d}", "Vị trí": hạng, "Kỳ": st.session_state.ky_quay})
    
    # Cập nhật Root và Khan
...
# --- 6. UI ---
st.markdown("<div style='text-align: center; font-size:22px; font-weight:bold; color:#1E3A8A;'>💎 HỆ THỐNG 18 BIẾN PRO</div>", unsafe_allow_html=True)
...
# NHẬP LIỆU
c_in1, c_in2, c_in3 = st.columns([1.5, 1.5, 1.8])
with c_in1: st.text_input("GĐB vừa nổ:", value="000000", key="gdb_in")
with c_in2: st.text_input("Ngày (ddmmyyyy):", value="10052026", key="date_in")
with c_in3: st.number_input("Kỳ quay (full):", value=st.session_state.ky_quay, key="ky_quay", step=1)
...
final_calc = get_matrix_scores()
df_final = pd.DataFrame(final_calc).sort_values(by=["d", "s"])

with tabs[0]:
    st.write("**⚡ LẤY QUÂN CÓ ĐIỂM B THẤP NHẤT**")
...
with tabs[1]:
    ...
    show_split("BO (15)", "bo", None, list(BO_MAP.keys()))
    show_split("GIAP (12)", "giap", None, list(GIAP_12.keys()))
    show_split("KEP/CACH (5)", "dang5", None, list(DANG_5.keys()))
    show_split("CHAN LE (4)", "cl4", None, list(CL_4.keys()))
    show_split("BE TO (4)", "bt4", None, list(BT_4.keys()))
...
with tabs[2]:
    st.write("**🎲 BẢNG B: TỔNG ĐIỂM (KHAN + ROOT) CỦA 100 SỐ**")
    m_data = [[next(x['d'] for x in final_calc if x['s'] == f"{d}{du}") for du in range(10)] for d in range(10)]
    st.table(pd.DataFrame(m_data, columns=[str(i) for i in range(10)], index=[str(i) for i in range(10)]))

with tabs[3]:
    for n, rv in [("NGÀY", rd), ("KỲ", rk), ("GĐB", rg)]:
        if rv in ROOT_DATA:
...
with tabs[6]:
    ...
    all_save = {k: st.session_state[k] for k in st.session_state.keys() if isinstance(st.session_state[k], (list, dict, int, float))}
    st.download_button("📥 TẢI DỮ LIỆU (.json)", data=json.dumps(all_save), file_name=f"18bien_final.json", use_container_width=True)
