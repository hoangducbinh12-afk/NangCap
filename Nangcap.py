...
# --- 1. CẤU HÌNH GIAO DIỆN & ANTI-TRANSLATE ---
...
st.markdown("""
    ...
    /* Fix nut bam +/- nho gọn */
    .small-btn button { height: 32px !important; width: 100% !important; padding: 0px !important; font-size: 16px !important; }
    /* Fix o nhap lieu */
    ...
    .root-display { font-size: 11px; font-weight: bold; color: #d32f2f; text-align: center; background: #fff5f5; padding: 5px; border-radius: 5px; margin-bottom: 10px; border: 1px solid #ffe3e3; }
    </style>
    <html lang="vi" class="notranslate" translate="no"></html>
    """, unsafe_allow_html=True)

# --- 2. DỮ LIỆU NHÓM SỐ (THEO MÀY CUNG CẤP) ---
...
# --- 5. LOGIC CẬP NHẬT TỔNG LỰC ---
def cap_nhat_tong_luc():
    ...
    # Update Khan Bien mo rong
    b_idx, c4_idx, bt4_idx, g_idx, d_idx = find_idx(n, BO_MAP), find_idx(n, CHAN_LE_4), find_idx(n, BE_TO_4), find_idx(n, GIAP_12), find_idx(n, DANG_5)
    for i in range(15): st.session_state.bo[i] = 0 if i==b_idx else st.session_state.bo[i]+1
    for i in range(4): st.session_state.cl4[i] = 0 if i==c4_idx else st.session_state.cl4[i]+1
    for i in range(4): st.session_state.bt4[i] = 0 if i==bt4_idx else st.session_state.bt4[i]+1
    for i in range(12): st.session_state.giap[i] = 0 if i==g_idx else st.session_state.giap[i]+1
    for i in range(5): st.session_state.dang[i] = 0 if i==d_idx else st.session_state.dang[i]+1
    # Update 8 bien phu 50/50
...
# --- 6. UI CHÍNH ---
st.markdown("<div class='main-title notranslate'>💎 HE THONG 18 BIEN PRO - TOAN DIEN</div>", unsafe_allow_html=True)
...
st.markdown(f"<div class='root-display notranslate'>Root: Ngay {st.session_state.rd} | Ky {st.session_state.rk} | GDB {st.session_state.rg}</div>", unsafe_allow_html=True)
st.button("🔥 CAP NHAT TONG LUC", on_click=cap_nhat_tong_luc, type="primary", use_container_width=True)

tabs = st.tabs(["⚡ Dan", "📊 Bang A", "🔢 Root", "🛠️ Sua"])

with tabs[0]:
    # Tinh toan ma tran 100 so
...
with tabs[1]:
    ...
    for lbl, k, cat in [("DAU","dau","dau"),("DUOI","duoi","duoi"),("TONG","tong","tong"),("HIEU","hieu","hieu"),("CHAM","cham","cham")]:
        show_split(lbl, k, cat, range(10))
    show_split("BO", "bo", None, [x for x in BO_MAP.keys()])
    show_split("CHAN LE 4", "cl4", None, list(CHAN_LE_4.keys()))
    show_split("BE TO 4", "bt4", None, list(BE_TO_4.keys()))
    show_split("12 GIAP", "giap", None, list(GIAP_12.keys()))
    show_split("DANG 5", "dang", None, list(DANG_5.keys()))
...
with tabs[3]:
    ...
    for k, lbl in [('bo','BO'),('cl4','CHANLE4'),('bt4','BETO4'),('giap','GIAP'),('dang','DANG')]:
        with st.expander(f"SUA {lbl}"):
            cols = st.columns(4)
