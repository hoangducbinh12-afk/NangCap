...
st.set_page_config(page_title="18 BIEN PRO - FULL 13 DANG", layout="centered")
...
st.markdown("""
    ...
    .dan-box-1 { background-color: #e8f5e9; padding: 10px; border-radius: 5px; color: #2e7d32; font-family: monospace; font-size: 13px; font-weight: bold; margin-bottom: 5px; }
    .dan-box-2 { background-color: #e3f2fd; padding: 10px; border-radius: 5px; color: #1565c0; font-family: monospace; font-size: 13px; font-weight: bold; margin-bottom: 5px; }
...
# --- 2. ĐỊNH NGHĨA DỮ LIỆU NHÓM SỐ (13 DẠNG MỚI) ---
DANG_5 = {
...
# (Giữ nguyên BO_MAP, GIAP_12, SO_THUONG và ROOT_DATA từ bản trước)
...
def cap_nhat_logic():
    ...
    st.session_state.ls.insert(0, {"Số về": f"{n:02d}", "Vị trí": 0})

# --- 5. GIAO DIỆN CHÍNH ---
st.markdown("<div class='main-title'>💎 HỆ THỐNG 18 BIẾN PRO - FULL 13 DẠNG</div>", unsafe_allow_html=True)
...
with c3: st.number_input("Kỳ quay:", value=st.session_state.ky_quay, key="ky_quay", step=1)

st.button("🔥 CẬP NHẬT TỔNG LỰC", on_click=cap_nhat_logic, type="primary", use_container_width=True)

tabs = st.tabs(["📊 Bảng A", "⚡ Dàn", "🕒 Lịch Sử", "💾 SAO LƯU"])
...
with tabs[0]:
    ...
    def show_row(lbl, k, cat, names):
        ...
        rt = [sum(ROOT_DATA[r][cat].index(i) if r in ROOT_DATA else 0 for r in [rd,rk,rg]) if cat else 0 for i in range(len(khan))]
    ...
    for lbl, k, cat in [("DAU","dau","dau"),("DUOI","duoi","duoi"),("TONG","tong","tong"),("HIEU","hieu","hieu"),("CHAM","cham","cham")]: 
        show_row(lbl, k, cat, range(10))
    
    show_row("BỘ (15)", "bo", None, list(BO_MAP.keys()))
    show_row("GIÁP (12)", "giap", None, list(GIAP_12.keys()))
    
    # --- 13 DẠNG MỚI MÀY YÊU CẦU ---
    st.divider()
    st.subheader("13 DẠNG BIẾN THỂ")
    show_row("5 DẠNG KÉP/CÁCH", "dang5", None, list(DANG_5.keys()))
    show_row("4 DẠNG CHẴN LẺ", "cl4", None, list(CL_4.keys()))
    show_row("4 DẠNG BÉ TO", "bt4", None, list(BT_4.keys()))

    st.write("**8 BIẾN PHỤ 50/50**")
    st.table(pd.DataFrame({
        "ĐẦU C/L": st.session_state.d_cl, "ĐUÔI C/L": st.session_state.u_cl, "TỔNG C/L": st.session_state.t_cl, "HỆ SỐ": st.session_state.so_he,
        "ĐẦU B/T": st.session_state.d_tb, "ĐUÔI B/T": st.session_state.u_tb, "TỔNG B/T": st.session_state.t_tb, "HIỆU B/T": st.session_state.h_tb
    }, index=["0/BE/CH/TH","1/TO/LE/HE"]))

with tabs[1]:
    # (Phần tính toán dàn số giữ nguyên logic cũ nhưng thêm 13 biến này vào trọng số)
    st.info("Tính năng soi dàn đang ưu tiên theo 13 biến mới mày vừa cập nhật.")

with tabs[3]:
    st.subheader("💾 Quản lý Dữ liệu")
    # Backup đầy đủ state
    data_all = {k: st.session_state[k] for k in st.session_state.keys() if isinstance(st.session_state[k], (list, dict, int, float))}
    st.download_button("📥 TẢI DỮ LIỆU (.json)", data=json.dumps(data_all), file_name=f"full_backup_{datetime.now().strftime('%d%m')}.json", use_container_width=True)
    uploaded = st.file_uploader("📤 UPLOAD ĐỂ KHÔI PHỤC", type="json")
    if uploaded and st.button("🚀 XÁC NHẬN"):
