...
st.markdown("""
    ...
    .section-title { font-size: 13px; font-weight: bold; color: #1E3A8A; margin-top: 15px; border-left: 4px solid #1E3A8A; padding-left: 5px; margin-bottom: 5px; }
    </style>
    """, unsafe_allow_html=True)

# --- DỮ LIỆU ROOT DATA (TỪ TRÁI SANG PHẢI 0-9Đ) ---
ROOT_DATA = {
...
# --- HÀM CẬP NHẬT CHỈ CHẠY KHI NHẤN NÚT ---
def cap_nhat_tong_luc():
    ...
    # Tính và lưu Root vào session_state để không tự nhảy
...
# Chỉ hiện Root sau khi bấm Update
...
st.button("🔥 CẬP NHẬT TỔNG LỰC", on_click=cap_nhat_tong_luc, type="primary", use_container_width=True)

t1, t2, t3, t4, t5 = st.tabs(["⚡ Dàn", "📊 Bảng A", "🔢 Kiểm Tra Root", "🎲 Ma Trận B", "🕒 LS"])

with t1:
    c_n1, c_n2 = st.columns(2)
    with c_n1: num_1 = st.number_input("Số quân Dàn 1:", 1, 100, 10)
    with c_n2: num_2 = st.number_input("Số quân Dàn 2:", 1, 100, 36)
    ...
    st.success(f"Dàn {num_1} quân thấp nhất: " + ", ".join(df_f.head(num_1)["s"].tolist()))
    st.info(f"Dàn {num_2} quân thấp nhất: " + ", ".join(df_f.head(num_2)["s"].tolist()))
...
with t4:
    ...
    for d in range(10):
        ...
        for du in range(10):
            ...
            sr = sum(ROOT_DATA[r][c].index(v) if r in ROOT_DATA else 0 for r in [rd,rk,rg] for c, v in [("đầu",d),("đuôi",du),("tổng",t),("hiệu",h),("chạm",d),("chạm",du)])
            row.append(sk + sr)
        m_data.append(row)
