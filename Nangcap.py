...
st.markdown("""
    ...
    .root-label { font-size: 12px; font-weight: bold; color: #d32f2f; text-align: center; background: #fdf2f2; padding: 5px; border-radius: 5px; margin-bottom: 5px; }
    .section-title { font-size: 13px; font-weight: bold; color: #1E3A8A; margin-top: 5px; border-left: 4px solid #1E3A8A; padding-left: 5px; }
    
    /* Box dàn số chuẩn màu */
    .dan-box-1 { background-color: #e8f5e9; padding: 8px; border-radius: 5px; border: 1px solid #c8e6c9; color: #2e7d32; font-family: monospace; font-size: 12px; font-weight: bold; }
    .dan-box-2 { background-color: #e3f2fd; padding: 8px; border-radius: 5px; border: 1px solid #bbdefb; color: #1565c0; font-family: monospace; font-size: 12px; font-weight: bold; }
    
    /* Nút bấm nhỏ gọn */
    .stButton>button { height: 30px !important; font-size: 12px !important; }
    </style>
...
with c2: st.text_input("Ngày:", value="", key="date_in", placeholder="09092009")
with c3:
    st.write("Kỳ quay:")
...
with t1:
    ...
    with col_n1: num_1 = st.number_input("Quân Dàn 1:", 1, 100, 10, key="n1")
    with col_n2: num_2 = st.number_input("Quân Dàn 2:", 1, 100, 36, key="n2")
    ...
    d1_str = ", ".join(df_f.head(num_1)["s"].tolist())
    d2_str = ", ".join(df_f.head(num_2)["s"].tolist())

    st.markdown(f"**Dàn {num_1} (Thấp nhất):**")
    st.markdown(f"<div class='dan-box-1'>{d1_str}</div>", unsafe_allow_html=True)
    if st.button("📋 COPY DÀN 1"): st.write(f'<script>navigator.clipboard.writeText("{d1_str}")</script>', unsafe_allow_html=True); st.toast("Đã copy!")
    
    st.markdown(f"**Dàn {num_2} (Thấp nhất):**")
    st.markdown(f"<div class='dan-box-2'>{d2_str}</div>", unsafe_allow_html=True)
    if st.button("📋 COPY DÀN 2"): st.write(f'<script>navigator.clipboard.writeText("{d2_str}")</script>', unsafe_allow_html=True); st.toast("Đã copy!")

with t2:
    st.write("**BẢNG A (KHAN + ROOT - KHÔNG DẤU)**")
    rd, rk, rg = st.session_state.rd, st.session_state.rk, st.session_state.rg
    ...
    sub_data = {
        "DAU C/L": st.session_state.d_cl, "DUOI C/L": st.session_state.u_cl,
        "TONG C/L": st.session_state.t_cl, "DAU B/T": st.session_state.d_tb,
        "DUOI B/T": st.session_state.u_tb, "TONG B/T": st.session_state.t_tb,
...
with t3:
    ...
    m_data = []
    for d in range(10):
        row = [next(x for x in f_list if x["s"] == f"{d}{du}")["d"] for du in range(10)]
        m_data.append(row)
...
with t4:
    ...
    with c_a:
        for i in range(10): st.session_state.dau[i] = st.number_input(f"DAU {i}", value=st.session_state.dau[i], key=f"sd_{i}")
    with c_b:
        for i in range(10): st.session_state.duoi[i] = st.number_input(f"DUOI {i}", value=st.session_state.duoi[i], key=f"su_{i}")
