...
# --- CAU HINH GIAO DIEN (CHONG TU DONG DICH) ---
st.set_page_config(page_title="18 BIEN PRO - TOAN DIEN", layout="centered")

# Ma CSS tong luc de fix giao dien
st.markdown("""
    <html lang="vi" class="notranslate" translate="no">
    <style>
    /* Khoa tinh nang tu dong dich cua trinh duyet */
    ...
    /* Fix font trong o nhap lieu */
    div[data-baseweb="input"] { height: 30px !important; }
    </style>
    </html>
...
st.markdown(f"<div class='root-label notranslate'>Root Ngay: {st.session_state.rd} | Root Ky: {st.session_state.rk} | Root GDB: {st.session_state.rg}</div>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with col_d1:
        ...
        st.markdown(f"<div class='dan-box-1 notranslate'>{d1_s}</div>", unsafe_allow_html=True)
    ...
    with col_d2:
        ...
        st.markdown(f"<div class='dan-box-2 notranslate'>{d2_s}</div>", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    show_r("BO", "bo", None, [x for x in BO_MAP.keys()])
